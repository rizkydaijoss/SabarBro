...
public class AdapterRekomendasi extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private Context context;
    private LayoutInflater inflater;
    List<DataRestoran> data= Collections.emptyList();


    // create constructor to innitilize context and data sent from FragmentList
    public AdapterRekomendasi(Context context, List<DataRestoran> data){
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        DataRestoran current=data.get(position);
        ...
        myHolder.hp1.setText(current.getHp());
        myHolder.hp1.setText("Call: " + current.getHp());

        // load image into imageview using glide
        Glide.with(context).load("http://masgandos.pe.hu/sabarbro/rekomendasi/" + current.getGambar())
